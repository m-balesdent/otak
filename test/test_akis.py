...
def test_akis():
    my_non_lin_osc = ot.PythonFunction(6, 1, non_lin_osc)
    vect = ot.RandomVector(dist_x)
    G = ot.CompositeRandomVector(my_non_lin_osc, vect)
    ...
    assert my_AK_non_lin_osc.getFailureProbability()== pytest.approx(0.0287,abs=1e-4)